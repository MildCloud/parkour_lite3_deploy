...
public:
    StateMachine(RobotType robot_type){
        ...
        if(robot_type == RobotType::Lite3){
            ...
            const std::string urdf_path = GetAbsPath()+"/../third_party/URDF_model/x30_urdf/X30/urdf/X30.urdf";
            #ifdef USE_RAISIM
                ri_ptr_ = std::make_shared<JueyingRaisimSimulation>(activation_key, urdf_path);
            #else
            ...
            const std::string urdf_path = GetAbsPath()+"/../../robot_model/P50/urdf/P50.urdf";
            #ifdef USE_RAISIM
                ri_ptr_ = std::make_shared<JueyingRaisimSimulation>(activation_key, urdf_path);
            #else
